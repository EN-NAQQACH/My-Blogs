...
function EditBlog({ id }) {
    ...
    useEffect(() => {
        const getBlog = async () => {
            try {
                const res = await fetch(`/api/blog/getablog?id=${id}`)
                const data = await res.json()
                if (res.ok) {
        ...
        <div className='p-[60px] min-h-lvh m-auto  '>
            <div className='mt-10 mb-52'>
                <div className='flex justify-end gap-3'>
                    <button onClick={() => handleUpdateBlog()} className='pr-5 pl-5 bg-black text-white pt-1 pb-1 rounded-3xl text-[13px]'>Update</button>
                    <Link href={'/dashboard/blogs'} className='pr-5 pl-5 bg-gray-50 text-black pt-1 pb-1 rounded-3xl text-[13px]'>Cancel</Link>
                </div>
